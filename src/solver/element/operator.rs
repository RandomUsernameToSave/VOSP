use crate::parameters::{DX,dxi,epsi,NV,NX};
use super::Element;
...
impl Element {
    ...
    pub fn x_advection(&mut self,DT:f64) {
        ...
        for ix in 0..NX {
            for iv  in 0..NV {
                ...
                calc_ind = ix as isize- (v1*DT*dxi ).ceil() as isize;
                //calc_ind -= 1;
                th = ((v1*DT*dxi).ceil() as isize) as f64 - v1*DT*dxi;

                if calc_ind <= 0 {
                    self.element_grid[ix][iv] = self.boundary_conditions("lx",ix,iv);
                }
                else if calc_ind >= ((NX-2) as isize) {
                ...
                r1 =    betal * dxi * (cached_grid[start_ind][iv] - cached_grid[start_ind-1][iv])  + 
                        betar * dxi * (cached_grid[start_ind+1][iv] - cached_grid[start_ind][iv]) + 
                        betac * 0.5*dxi * (cached_grid[start_ind+1][iv] - cached_grid[start_ind-1][iv]);
                r2 = 2.* betac * dxi*dxi*((cached_grid[start_ind+1][iv] - 2.* cached_grid[start_ind][iv] + cached_grid[start_ind-1][iv]));
                ...
                r1p =    betal * dxi * (cached_grid[start_ind][iv] - cached_grid[start_ind-1][iv])  + 
                        betar * dxi * (cached_grid[start_ind+1][iv] - cached_grid[start_ind][iv]) + 
                        betac * 0.5*dxi * (cached_grid[start_ind+1][iv] - cached_grid[start_ind-1][iv]);
                r2p = 2.* betac * dxi*dxi*((cached_grid[start_ind+1][iv] - 2.* cached_grid[start_ind][iv] + cached_grid[start_ind-1][iv]));
                ...
                t0 =(1.-th)*r0 + th*r0p;
                t1 = DX* th*(1.-th) * (0.5*r1 - 0.5 * r1p);
                q = 3.*th - 6.*th*th + 4. * th*th*th;
                t2 = DX*DX /24.* ( (1.-q)*r2 +q*r2p);
    ...
    pub fn v_advection(&mut self,DT:f64, elec_field: &Vec<f64>) {
        ...
        let cached_grid = self.element_grid.clone();
        let dvi:f64 = 2.* self.lv/(NV as f64);
        ...
        for ix in 0..NX {
            for iv  in 0..NV {
                ...
                else if calc_ind >= ((NV-2) as isize) {
